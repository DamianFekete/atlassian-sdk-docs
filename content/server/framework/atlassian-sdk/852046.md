...
First upgrade the host application to a version which supports version 2 of the plugin framework. For example, upgrade to Confluence 2.10 or later, or JIRA 4.0 or later. See the [Plugin Framework Version Matrix](/server/framework/atlassian-sdk/plugin-framework-version-matrix-852041.html).
...
The plugin framework handles all the OSGi requirements, such as bundle instructions, manifest, etc. See [Going from Plugin to OSGi Bundle](/server/framework/atlassian-sdk/going-from-plugin-to-osgi-bundle-852147.html). For more complex plugins, you may find it useful to do more complex configuration, such as supplying complete Spring configuration files.
...
-   [Using the OSGi Browser](/server/framework/atlassian-sdk/using-the-osgi-browser-8946139.html) to determine the packages and components exposed by an application.
...
See [Managing Dependencies](/server/framework/atlassian-sdk/managing-dependencies-2818370.html).
