---
aliases:
- /server/framework/atlassian-sdk/plugin-framework-faq-852039.html
- /server/framework/atlassian-sdk/plugin-framework-faq-852039.md
category: devguide
confluence_id: 852039
dac_edit_link: https://developer.atlassian.com/pages/editpage.action?cjm=wozere&pageId=852039
dac_view_link: https://developer.atlassian.com/pages/viewpage.action?cjm=wozere&pageId=852039
date: '2017-12-11'
legacy_title: Plugin Framework FAQ
platform: server
product: atlassian-sdk
subcategory: faq
title: Plugin Framework FAQ
---
# Plugin Framework FAQ

-   [Access components statically](/server/framework/atlassian-sdk/access-components-statically)
-   [Accessing Classes from Another Plugin](/server/framework/atlassian-sdk/accessing-classes-from-another-plugin)
-   [Differences between Spring and Pico](/server/framework/atlassian-sdk/differences-between-spring-and-pico)
-   [Differences between Version 1 Plugins and Version 2 Plugins](/server/framework/atlassian-sdk/differences-between-version-1-plugins-and-version-2-plugins)
-   [Marking Packages as Optional Imports](/server/framework/atlassian-sdk/marking-packages-as-optional-imports)
-   [My public component just disappeared!](/server/framework/atlassian-sdk/my-public-component-just-disappeared)
-   [Plugin Modules that Cannot be Disabled](/server/framework/atlassian-sdk/plugin-modules-that-cannot-be-disabled)
-   [Plugin Modules that Require a Restart](/server/framework/atlassian-sdk/plugin-modules-that-require-a-restart)
-   [Running on WebSphere 6.1](/server/framework/atlassian-sdk/running-on-websphere-6-1)
-   [Using JDOM in OSGi](/server/framework/atlassian-sdk/using-jdom-in-osgi)
-   [What happened to studio.plugins.atlassian.com?](/server/framework/atlassian-sdk/what-happened-to-studio-plugins-atlassian-com)
















































































































































































































































