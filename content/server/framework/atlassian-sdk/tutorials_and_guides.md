
---
aliases:

category: devguide
confluence_id:
dac_edit_link:
dac_view_link:
legacy_url:
new_url: /server/framework/atlassian-sdk/tutorials_and_guides/
platform: server
product: atlassian-sdk
subcategory: learning
title: Tutorials and guides
---        

## Tutorials


[Getting started with Active Objects](/server/framework/atlassian-sdk/getting-started-with-active-objects)

[Installing Active Objects](/server/framework/atlassian-sdk/installing-active-objects)

[Making pluggable inline actions for Activity Streams](/server/framework/atlassian-sdk/making-pluggable-inline-actions-for-activity-streams)

[Making your own Activity Streams provider](/server/framework/atlassian-sdk/making-your-own-activity-streams-provider)

[Exposing host components via Spring](/server/framework/atlassian-sdk/exposing-host-components-via-spring)

[Quick start guide to embedding](/server/framework/atlassian-sdk/quick-start-guide-to-embedding)

[Developing a REST service plugin](/server/framework/atlassian-sdk/developing-a-rest-service-plugin)

[Learn the development platform by example](/server/framework/atlassian-sdk/learn-the-development-platform-by-example)

[Control access with SAL](/server/framework/atlassian-sdk/control-access-with-sal)

[Convert component to servlet module](/server/framework/atlassian-sdk/convert-component-to-servlet-module)

[Create a GUI with templates and AUI](/server/framework/atlassian-sdk/create-a-gui-with-templates-and-aui)

[Create a plugin skeleton](/server/framework/atlassian-sdk/create-a-plugin-skeleton)

[Run your plugin in the container](/server/framework/atlassian-sdk/run-your-plugin-in-the-container)

[Store and retrieve plugin data](/server/framework/atlassian-sdk/store-and-retrieve-plugin-data)

[Set up the Atlassian Plugin SDK and build a project](/server/framework/atlassian-sdk/set-up-the-atlassian-plugin-sdk-and-build-a-project)

[Create a Confluence 'Hello World' macro](/server/framework/atlassian-sdk/create-a-confluence-hello-world-macro)

[Create a HelloWorld plugin project](/server/framework/atlassian-sdk/create-a-helloworld-plugin-project)

[Explore the installed SDK and the atlas commands](/server/framework/atlassian-sdk/explore-the-installed-sdk-and-the-atlas-commands)

[Install the Atlassian SDK on a Linux or Mac system](/server/framework/atlassian-sdk/install-the-atlassian-sdk-on-a-linux-or-mac-system)

[Install the Atlassian SDK on a Windows system](/server/framework/atlassian-sdk/install-the-atlassian-sdk-on-a-windows-system)

[Modify the plugin](/server/framework/atlassian-sdk/modify-the-plugin)

[Modify the plugin using QuickReload](/server/framework/atlassian-sdk/modify-the-plugin-using-quickreload)

[Create and run traditional integration tests](/server/framework/atlassian-sdk/create-and-run-traditional-integration-tests)

[Create and run unit tests](/server/framework/atlassian-sdk/create-and-run-unit-tests)

[Create test data and a test fixture](/server/framework/atlassian-sdk/create-test-data-and-a-test-fixture)

[Generate and Examine skeleton tests](/server/framework/atlassian-sdk/generate-and-examine-skeleton-tests)

[Run Wired Tests with the Plugin Test Console](/server/framework/atlassian-sdk/run-wired-tests-with-the-plugin-test-console)


## Guides


[Adding data providers to your plugin](/server/framework/atlassian-sdk/adding-data-providers-to-your-plugin)

[Best Practices](/server/framework/atlassian-sdk/best-practices)

[Checking Plugins for API Compatibility](/server/framework/atlassian-sdk/checking-plugins-for-api-compatibility)

[Configuration of Instructions in Atlassian Plugins](/server/framework/atlassian-sdk/configuration-of-instructions-in-atlassian-plugins)

[Configure AMPS to run JIRA Core with additional applications installed](/server/framework/atlassian-sdk/configure-amps-to-run-jira-core-with-additional-applications-installed)

[Converting from Version 1 to Version 2 (OSGi) Plugins](/server/framework/atlassian-sdk/converting-from-version-1-to-version-2-osgi-plugins)

[Creating plugin module instances](/server/framework/atlassian-sdk/creating-plugin-module-instances)

[Speakeasy](/server/framework/atlassian-sdk/speakeasy)

[Developing Speakeasy extensions](/server/framework/atlassian-sdk/developing-speakeasy-extensions)

[Example extensions](/server/framework/atlassian-sdk/example-extensions)

[Extending the Speakeasy plugin](/server/framework/atlassian-sdk/extending-the-speakeasy-plugin)

[Installing Speakeasy](/server/framework/atlassian-sdk/installing-speakeasy)

[Troubleshooting Speakeasy](/server/framework/atlassian-sdk/troubleshooting-speakeasy)

[Stateless web-resource transforms and conditions](/server/framework/atlassian-sdk/stateless-web-resource-transforms-and-conditions)

[Supporting minification of JavaScript and CSS resources](/server/framework/atlassian-sdk/supporting-minification-of-javascript-and-css-resources)

[Working with Maven](/server/framework/atlassian-sdk/working-with-maven)

[Atlassian Platform common components](/server/framework/atlassian-sdk/atlassian-platform-common-components)

[About the Platform](/server/framework/atlassian-sdk/about-the-platform)

[Active Objects](/server/framework/atlassian-sdk/active-objects)

[AO 0.18.x upgrade guide](/server/framework/atlassian-sdk/ao-0-18-x-upgrade-guide)

[AO 0.29.x upgrade guide](/server/framework/atlassian-sdk/ao-0-29-x-upgrade-guide)

[How to upgrade AO to 0.18.x in JIRA 5](/server/framework/atlassian-sdk/how-to-upgrade-ao-to-0-18-x-in-jira-5)

[AO 1.0.0 upgrade guide](/server/framework/atlassian-sdk/ao-1-0-0-upgrade-guide)

[AO 1.1.0 upgrade guide](/server/framework/atlassian-sdk/ao-1-1-0-upgrade-guide)

[AO 1.2.0 upgrade guide](/server/framework/atlassian-sdk/ao-1-2-0-upgrade-guide)

[Developing your plugin with Active Objects](/server/framework/atlassian-sdk/developing-your-plugin-with-active-objects)

[Active Objects FAQ](/server/framework/atlassian-sdk/active-objects-faq)

[Column names](/server/framework/atlassian-sdk/column-names)

[Enabling SQL logging](/server/framework/atlassian-sdk/enabling-sql-logging)

[Table names](/server/framework/atlassian-sdk/table-names)

[Best practices for developing with Active Objects](/server/framework/atlassian-sdk/best-practices-for-developing-with-active-objects)

[Configuring the plugin](/server/framework/atlassian-sdk/configuring-the-plugin)

[Active Objects plugin module](/server/framework/atlassian-sdk/active-objects-plugin-module)

[Adding the maven dependency](/server/framework/atlassian-sdk/adding-the-maven-dependency)

[Configuring your project](/server/framework/atlassian-sdk/configuring-your-project)

[Testing a plugin that uses Active Objects](/server/framework/atlassian-sdk/testing-a-plugin-that-uses-active-objects)

[The Active Objects library](/server/framework/atlassian-sdk/the-active-objects-library)

[Creating an EntityManager](/server/framework/atlassian-sdk/creating-an-entitymanager)

[Creating entities](/server/framework/atlassian-sdk/creating-entities)

[Deleting entities](/server/framework/atlassian-sdk/deleting-entities)

[Finding entities](/server/framework/atlassian-sdk/finding-entities)

[Getting entities by PK](/server/framework/atlassian-sdk/getting-entities-by-pk)

[ManyToMany relationship](/server/framework/atlassian-sdk/manytomany-relationship)

[OneToMany relationship](/server/framework/atlassian-sdk/onetomany-relationship)

[OneToOne relationship](/server/framework/atlassian-sdk/onetoone-relationship)

[Testing](/server/framework/atlassian-sdk/testing)

[Upgrading your plugin and handling data model updates](/server/framework/atlassian-sdk/upgrading-your-plugin-and-handling-data-model-updates)

[Handling AO upgrade tasks](/server/framework/atlassian-sdk/handling-ao-upgrade-tasks)

[Activity Streams](/server/framework/atlassian-sdk/activity-streams)

[Adding Activities to a Third Party feed with the Java API](/server/framework/atlassian-sdk/adding-activities-to-a-third-party-feed-with-the-java-api)

[Adding Activities to a Third Party Feed with the REST API](/server/framework/atlassian-sdk/adding-activities-to-a-third-party-feed-with-the-rest-api)

[Consuming an Activity Streams feed](/server/framework/atlassian-sdk/consuming-an-activity-streams-feed)

[Application links](/server/framework/atlassian-sdk/application-links)

[Atlassian Template Renderer](/server/framework/atlassian-sdk/atlassian-template-renderer)

[Plugin Framework](/server/framework/atlassian-sdk/plugin-framework)

[Advanced configuration with Spring XML files](/server/framework/atlassian-sdk/advanced-configuration-with-spring-xml-files)

[Behind the scenes in the Plugin Framework](/server/framework/atlassian-sdk/behind-the-scenes-in-the-plugin-framework)

[Automatic generation of Spring configuration](/server/framework/atlassian-sdk/automatic-generation-of-spring-configuration)

[Going from Plugin to OSGi bundle](/server/framework/atlassian-sdk/going-from-plugin-to-osgi-bundle)

[Lifecycle of a bundle](/server/framework/atlassian-sdk/lifecycle-of-a-bundle)

[Embedding the Plugin Framework](/server/framework/atlassian-sdk/embedding-the-plugin-framework)

[REST API development](/server/framework/atlassian-sdk/rest-api-development)

[Atlassian REST API design guidelines version 1](/server/framework/atlassian-sdk/atlassian-rest-api-design-guidelines-version-1)

[Documenting your APIs with the Atlassian REST API Browser](/server/framework/atlassian-sdk/documenting-your-apis-with-the-atlassian-rest-api-browser)

[Scheduling events via SAL tutorial](/server/framework/atlassian-sdk/scheduling-events-via-sal-tutorial)

[Common coding tasks](/server/framework/atlassian-sdk/common-coding-tasks)

[Adding a configuration UI for your plugin](/server/framework/atlassian-sdk/adding-a-configuration-ui-for-your-plugin)

[Adding WebSudo support to your plugin](/server/framework/atlassian-sdk/adding-websudo-support-to-your-plugin)

[Creating an admin configuration form](/server/framework/atlassian-sdk/creating-an-admin-configuration-form)

[Development cycle](/server/framework/atlassian-sdk/development-cycle)

[Creating a plugin skeleton with the Atlassian SDK](/server/framework/atlassian-sdk/creating-a-plugin-skeleton-with-the-atlassian-sdk)

[Finding your plugin in the host application](/server/framework/atlassian-sdk/finding-your-plugin-in-the-host-application)

[Packaging and releasing your plugin](/server/framework/atlassian-sdk/packaging-and-releasing-your-plugin)

[Plugin release checklist](/server/framework/atlassian-sdk/plugin-release-checklist)

[Start a host application with a plugin installed](/server/framework/atlassian-sdk/start-a-host-application-with-a-plugin-installed)

[Internationalising your plugin](/server/framework/atlassian-sdk/internationalising-your-plugin)

[Pluralising internationalisation strings](/server/framework/atlassian-sdk/pluralising-internationalisation-strings)

[Storing plugin settings](/server/framework/atlassian-sdk/storing-plugin-settings)

[Using standard page decorators](/server/framework/atlassian-sdk/using-standard-page-decorators)

[Developer tools](/server/framework/atlassian-sdk/developer-tools)

[About the Atlassian RefApp](/server/framework/atlassian-sdk/about-the-atlassian-refapp)

[Additional developer resources](/server/framework/atlassian-sdk/additional-developer-resources)

[Early access programs](/server/framework/atlassian-sdk/early-access-programs)

[Automatic plugin reinstallation with FastDev](/server/framework/atlassian-sdk/automatic-plugin-reinstallation-with-fastdev)

[Automatic plugin reinstallation with QuickReload](/server/framework/atlassian-sdk/automatic-plugin-reinstallation-with-quickreload)

[Declaring JNDI datasources in AMPS](/server/framework/atlassian-sdk/declaring-jndi-datasources-in-amps)

[Using the OSGi Browser](/server/framework/atlassian-sdk/using-the-osgi-browser)

[Using the REST API Browser](/server/framework/atlassian-sdk/using-the-rest-api-browser)

[Working in an IDE](/server/framework/atlassian-sdk/working-in-an-ide)

[Configure IDEA to use the SDK](/server/framework/atlassian-sdk/configure-idea-to-use-the-sdk)

[Configure NetBeans to use the SDK](/server/framework/atlassian-sdk/configure-netbeans-to-use-the-sdk)

[Creating a remote debug target](/server/framework/atlassian-sdk/creating-a-remote-debug-target)

[Working with the SDK](/server/framework/atlassian-sdk/working-with-the-sdk)

[About AMPS build options](/server/framework/atlassian-sdk/about-amps-build-options)

[AMPS build configuration reference](/server/framework/atlassian-sdk/amps-build-configuration-reference)

[Changing the default Maven version](/server/framework/atlassian-sdk/changing-the-default-maven-version)

[Command reference](/server/framework/atlassian-sdk/command-reference)

[atlas-clean](/server/framework/atlassian-sdk/atlas-clean)

[atlas-cli](/server/framework/atlassian-sdk/atlas-cli)

[atlas-clover](/server/framework/atlassian-sdk/atlas-clover)

[atlas-compile](/server/framework/atlassian-sdk/atlas-compile)

[atlas-create-bamboo-plugin](/server/framework/atlassian-sdk/atlas-create-bamboo-plugin)

[atlas-create-bamboo-plugin-module](/server/framework/atlassian-sdk/atlas-create-bamboo-plugin-module)

[atlas-create-confluence-plugin](/server/framework/atlassian-sdk/atlas-create-confluence-plugin)

[atlas-create-confluence-plugin-module](/server/framework/atlassian-sdk/atlas-create-confluence-plugin-module)

[atlas-create-crowd-plugin](/server/framework/atlassian-sdk/atlas-create-crowd-plugin)

[atlas-create-crowd-plugin-module](/server/framework/atlassian-sdk/atlas-create-crowd-plugin-module)

[atlas-create-fecru-plugin](/server/framework/atlassian-sdk/atlas-create-fecru-plugin)

[atlas-create-fecru-plugin-module](/server/framework/atlassian-sdk/atlas-create-fecru-plugin-module)

[atlas-create-home-zip](/server/framework/atlassian-sdk/atlas-create-home-zip)

[atlas-create-jira-plugin](/server/framework/atlassian-sdk/atlas-create-jira-plugin)

[atlas-create-jira-plugin-module](/server/framework/atlassian-sdk/atlas-create-jira-plugin-module)

[atlas-create-refapp-plugin](/server/framework/atlassian-sdk/atlas-create-refapp-plugin)

[atlas-create-refapp-plugin-module](/server/framework/atlassian-sdk/atlas-create-refapp-plugin-module)

[atlas-debug](/server/framework/atlassian-sdk/atlas-debug)

[atlas-help](/server/framework/atlassian-sdk/atlas-help)

[atlas-install-plugin](/server/framework/atlassian-sdk/atlas-install-plugin)

[atlas-integration-test](/server/framework/atlassian-sdk/atlas-integration-test)

[atlas-mvn](/server/framework/atlassian-sdk/atlas-mvn)

[atlas-package](/server/framework/atlassian-sdk/atlas-package)

[atlas-release](/server/framework/atlassian-sdk/atlas-release)

[atlas-release-rollback](/server/framework/atlassian-sdk/atlas-release-rollback)

[atlas-run](/server/framework/atlassian-sdk/atlas-run)

[atlas-run-cloud](/server/framework/atlassian-sdk/atlas-run-cloud)

[atlas-run-standalone](/server/framework/atlassian-sdk/atlas-run-standalone)

[atlas-unit-test](/server/framework/atlassian-sdk/atlas-unit-test)

[atlas-update](/server/framework/atlassian-sdk/atlas-update)

[atlas-version](/server/framework/atlassian-sdk/atlas-version)

[Frequently used commands](/server/framework/atlassian-sdk/frequently-used-commands)

[Getting help with the SDK](/server/framework/atlassian-sdk/getting-help-with-the-sdk)

[Working with the POM](/server/framework/atlassian-sdk/working-with-the-pom)

[Managing dependencies](/server/framework/atlassian-sdk/managing-dependencies)

[Setting the platform version dependency in the POM](/server/framework/atlassian-sdk/setting-the-platform-version-dependency-in-the-pom)